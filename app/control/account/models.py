...
class AccountQuotaSet:
    """Quota set — one window per mode (auto / fast / expert / heavy / grok_4_3 / build).

    ``heavy``    is ``None`` for basic and super accounts.
    ``grok_4_3`` is ``None`` for basic accounts (super/heavy only).
    ``build``    has an independent window on accounts that expose it.
    """
    ...
    build: QuotaWindow | None = None  # Grok Build agent mode (independent window)

    def get(self, mode_id: int) -> QuotaWindow | None:
        """Return the quota window for *mode_id* (0=auto, 1=fast, 2=expert, 3=heavy, 4=grok_4_3, 5=build)."""
        if mode_id == 0:
            return self.auto
        if mode_id == 1:
            return self.fast
    ...
    def from_dict(cls, d: dict[str, Any]) -> "AccountQuotaSet":
        ...
        return cls(
            ...
            build=QuotaWindow.from_dict(build_d) if build_d else None,
        )
