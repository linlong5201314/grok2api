...
推荐使用 Railway + Volume 的单实例部署方案。

> 参考部署模板：`railway.toml`
>
> 环境变量模板：`.env.railway.example`
>
> 完整说明：`docs/RAILWAY.md`
>
> 推荐挂载 Volume 到 `/app/data`，并设置 `SERVER_STORAGE_TYPE=local`。
>
> 单实例场景不需要 Redis。
...
> 配置 `.env` 文件，可直接参考 `.env.example` 或 `.env.railway.example`
...
| `SERVER_STORAGE_TYPE` | 存储类型（`local`/`redis`/`mysql`/`pgsql`） | `local` | `local` |
